package branch

import (
	"github.com/ironpark/teatime/node"
	"github.com/ironpark/teatime/node/types"
)

func init() {
	node.RegisterNode(&FilterBranchNode{})
}

// 데이터를 필터링하여 분기 처리하는 필터 브랜치 노드
type FilterBranchNode struct {
	customParams []types.NodeProperty
}

func (r *FilterBranchNode) Name() string {
	return "Filter"
}

func (r *FilterBranchNode) Description() string {
	return "데이터를 필터링하여 조건에 맞는 항목만 처리하는 필터 브랜치 노드입니다."
}

func (r *FilterBranchNode) Type() types.NodeType {
	return types.NodeTypeBranch
}

func (r *FilterBranchNode) ID() string {
	return "teatime.branch.filter"
}

func (r *FilterBranchNode) Output() []types.NodeProperty {
	return []types.NodeProperty{
		{
			Name:        "Filtered Items",
			Description: "필터링된 항목들",
			Key:         "filteredItems",
			Value:       "",
			Type:        types.JSONArray,
		},
		{
			Name:        "Rejected Items",
			Description: "필터링에서 제외된 항목들",
			Key:         "rejectedItems",
			Value:       "",
			Type:        types.JSONArray,
		},
		{
			Name:        "Filtered Count",
			Description: "필터링된 항목 수",
			Key:         "filteredCount",
			Value:       "",
			Type:        types.Int64,
		},
		{
			Name:        "Rejected Count",
			Description: "제외된 항목 수",
			Key:         "rejectedCount",
			Value:       "",
			Type:        types.Int64,
		},
		{
			Name:        "Total Count",
			Description: "전체 항목 수",
			Key:         "totalCount",
			Value:       "",
			Type:        types.Int64,
		},
		{
			Name:        "Filter Rate",
			Description: "필터링 비율 (%)",
			Key:         "filterRate",
			Value:       "",
			Type:        types.Float64,
		},
	}
}

func (r *FilterBranchNode) Properties() []types.NodeProperty {
	return []types.NodeProperty{
		{
			Name:        "Input Data",
			Description: "필터링할 입력 데이터 (배열)",
			Optional:    false,
			Key:         "inputData",
			Value:       "[]",
			Type:        types.JSON,
		},
		{
			Name:        "Filter Type",
			Description: "필터 타입",
			Optional:    false,
			Key:         "filterType",
			Value:       "simple",
			Type:        types.String,
			Options:     []string{"simple", "complex", "custom", "regex", "range", "unique", "duplicate"},
		},
		{
			Name:        "Filter Field",
			Description: "필터링할 필드명 (객체 배열의 경우)",
			Optional:    true,
			Key:         "filterField",
			Value:       "",
			Type:        types.String,
		},
		{
			Name:        "Filter Operator",
			Description: "필터 연산자",
			Optional:    true,
			Key:         "filterOperator",
			Value:       "==",
			Type:        types.String,
			Options:     []string{"==", "!=", ">", "<", ">=", "<=", "contains", "notContains", "startsWith", "endsWith", "in", "notIn", "between", "isNull", "isNotNull"},
		},
		{
			Name:        "Filter Value",
			Description: "필터 값",
			Optional:    true,
			Key:         "filterValue",
			Value:       "",
			Type:        types.Text,
		},
		{
			Name:        "Filter Expression",
			Description: "복잡한 필터 표현식 (custom 타입용)",
			Optional:    true,
			Key:         "filterExpression",
			Value:       "",
			Type:        types.Text,
		},
		{
			Name:        "Multiple Filters",
			Description: "다중 필터 조건 (JSON 배열)",
			Optional:    true,
			Key:         "multipleFilters",
			Value:       "[]",
			Type:        types.JSON,
		},
		{
			Name:        "Filter Logic",
			Description: "다중 필터 논리 연산",
			Optional:    true,
			Key:         "filterLogic",
			Value:       "AND",
			Type:        types.String,
			Options:     []string{"AND", "OR", "XOR", "CUSTOM"},
		},
		{
			Name:        "Case Sensitive",
			Description: "대소문자 구분",
			Optional:    true,
			Key:         "caseSensitive",
			Value:       "true",
			Type:        types.Bool,
		},
		{
			Name:        "Null Handling",
			Description: "NULL 값 처리 방식",
			Optional:    true,
			Key:         "nullHandling",
			Value:       "exclude",
			Type:        types.String,
			Options:     []string{"include", "exclude", "only"},
		},
		{
			Name:        "Sort After Filter",
			Description: "필터링 후 정렬 여부",
			Optional:    true,
			Key:         "sortAfterFilter",
			Value:       "false",
			Type:        types.Bool,
		},
		{
			Name:        "Sort Field",
			Description: "정렬 필드",
			Optional:    true,
			Key:         "sortField",
			Value:       "",
			Type:        types.String,
		},
		{
			Name:        "Sort Order",
			Description: "정렬 순서",
			Optional:    true,
			Key:         "sortOrder",
			Value:       "asc",
			Type:        types.String,
			Options:     []string{"asc", "desc"},
		},
		{
			Name:        "Limit",
			Description: "결과 제한 수",
			Optional:    true,
			Key:         "limit",
			Value:       "0",
			Type:        types.Int64,
		},
		{
			Name:        "Offset",
			Description: "시작 오프셋",
			Optional:    true,
			Key:         "offset",
			Value:       "0",
			Type:        types.Int64,
		},
	}
}

func (r *FilterBranchNode) CustomParams() []types.NodeProperty {
	return r.customParams
}

func (r *FilterBranchNode) AddCustomParam(param types.NodeProperty) {
	r.customParams = append(r.customParams, param)
}
