...
func (r *LLMActionNode) Properties() []node.NodeProperty {
	return []node.NodeProperty{
		...
		},
		{
			...
			Value:       "",
			Type:        node.String,
			Input: &node.InputConfig{
				Type:        node.InputTypeTextarea,
				Rows:        3,
		...
		},
		{
			...
			Type:        node.Float64,
			Input: &node.InputConfig{
				Type: node.InputTypeRange,
				Min:  node.Float64Ptr(0),
				Max:  node.Float64Ptr(2),
				Step: node.Float64Ptr(0.1),
			},
		},
		{
			Name:        "Max Tokens",
			...
			Type:        node.Int64,
			Input: &node.InputConfig{
				Type: node.InputTypeNumber,
				Min:  node.Float64Ptr(1),
				Max:  node.Float64Ptr(8192),
				Step: node.Float64Ptr(1),
			},
		},
		{
			Name:        "Top P",
